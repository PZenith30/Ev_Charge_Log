 ...
 */
import { budgetStatus, monthlyAvgByType, monthlyTotals, dueList, sDist, sEff, sKwh100, sTotal, summarize } from './calc';
import { BUDGET_TYPES, COST_CATS } from './data';
...
export function buildContext({
  ...
  // งบแยกชนิด — ส่งเฉพาะชนิดที่ตั้งงบไว้จริง ชนิดที่ไม่ได้ตั้งไม่มีอะไรให้ตอบ
  const avgType = monthlyAvgByType(sessions, costs);
  const budgetRows = budgetStatus(BUDGET_TYPES, settings.budgets, budget, avgType).rows.map((r) => ({
    ชนิด: r.label,
    งบต่อเดือน: r.budget,
    ใช้จริงเฉลี่ย: r2(r.avg),
    เกินงบ: r.over,
  }));
  ...
  return {
    ...
    งบประมาณรวมต่อเดือน: budget > 0 ? { ตั้งไว้: budget, ใช้จริงเฉลี่ย: r2(avgPerMonth), เกินงบ: avgPerMonth > budget } : 'ยังไม่ได้ตั้งงบ',
    งบประมาณแยกชนิดต่อเดือน: budgetRows.length ? budgetRows : 'ยังไม่ได้ตั้งงบแยกชนิด',
