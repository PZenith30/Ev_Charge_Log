...
/** ระยะทางที่วิ่งได้ = เลขไมล์ปัจจุบัน − เลขไมล์ครั้งก่อน */
...
/**
 * ระยะทางที่เพิ่มขึ้นตามที่รถแสดง = ระยะทางหลังชาร์จ − ระยะทางก่อนชาร์จ
 *
 * คนละเรื่องกับ sDist ซึ่งคือระยะทางที่ "ขับมาจริง" จากเลขไมล์
 * อันนี้คือตัวเลขคาดการณ์ของรถ (guess-o-meter) จึงติดลบได้ถ้ารถคำนวณใหม่แล้วมองโลกในแง่ร้ายลง
 * ไม่หนีบให้เป็นบวก เพราะค่าติดลบเป็นข้อมูลจริงที่ควรเห็น
 */
export function sRangeGain(s) {
  if (!isNum(s.rangeBefore) || !isNum(s.rangeAfter)) return null;
  return Number(s.rangeAfter) - Number(s.rangeBefore);
}
/** ค่าใช้จ่ายรวมต่อครั้ง — ใช้ค่าที่ผู้ใช้กรอกทับถ้ามี ไม่งั้นคำนวณจาก kWh × ราคา + ค่าบริการ */
/**
 * ค่าใช้จ่ายรวม — ใช้ยอดที่กรอกเองก่อนถ้ามี ไม่งั้นคำนวณจาก พลังงาน x ราคา + ค่าปรับ - ส่วนลด
 * หนีบไม่ให้ติดลบ เพราะส่วนลดที่มากกว่ายอดจริงแปลว่ากรอกผิด ไม่ใช่ได้เงินคืน
 */
export const sTotal = (s) =>
  (isNum(s.total)
    ? Number(s.total)
    : Math.max(0, n(s.kwh) * n(s.price) + n(s.fee) - n(s.discount)));
...
export function summarize(list) {
  const o = {
    count: list.length, kwh: 0, cost: 0, dist: 0, fee: 0, discount: 0, seconds: 0,
 ...
 *
 * items เก็บรายการดิบไว้ด้วย เพื่อให้หน้าจอกางดูรายการของสถานีนั้นและกดแก้ไขได้
 * ลำดับใน items เท่ากับลำดับของ list ที่ส่งเข้ามา — ถ้าส่งรายการที่เรียงใหม่ไปเก่า
 * items[0] ก็คือครั้งล่าสุดของสถานีนั้น
 */
export function groupStations(list) {
  const map = new Map();
  for (const s of list) {
    const key = (s.station || '').trim() || 'ไม่ระบุสถานี';
    if (!map.has(key)) map.set(key, { name: key, count: 0, kwh: 0, cost: 0, ac: 0, dc: 0, items: [] });
    const g = map.get(key);
    g.count += 1;
    g.kwh += n(s.kwh);
    g.cost += sTotal(s);
...
/**
 * ค่าใช้จ่ายเฉลี่ยต่อเดือน แยกตามชนิด
 *
 * ทุกชนิดหารด้วย "จำนวนเดือนที่มีข้อมูลอะไรก็ได้" ชุดเดียวกัน ไม่ใช่จำนวนเดือนของชนิดนั้นเอง
 * เพราะค่าประกันกับค่าภาษีจ่ายปีละครั้ง ถ้าหารด้วยเดือนที่มีรายการของตัวเอง
 * จะได้ "เฉลี่ยต่อเดือน" เท่ากับเบี้ยทั้งปี ซึ่งเอาไปเทียบกับงบต่อเดือนไม่ได้เลย
 * หารด้วยเดือนทั้งหมดจึงได้ภาระต่อเดือนจริงตามที่งบรายเดือนต้องการ
 *
 * ใช้ชุดเดือนเดียวกับ avgMonthlySpend เพื่อให้ยอดแยกชนิดรวมกันแล้วเท่ากับยอดรวม
 */
export function monthlyAvgByType(sessions, costs) {
  ...
  for (const c of costs) {
    ...
    months.add(m);
    const k = c.cat || 'other';
    sum[k] = (sum[k] || 0) + n(c.amount);
  }

  const div = months.size || 1;
  const out = { months: months.size, total: 0 };
...
/**
 * เทียบยอดเฉลี่ยต่อเดือนกับงบที่ตั้งไว้ ทั้งงบรวมและงบแยกชนิด
 *
 * @param types    รายการชนิดจาก BUDGET_TYPES
 * @param budgets  งบแยกชนิด { [key]: number } — 0 หรือไม่มีค่า = ไม่ได้ตั้งงบชนิดนั้น
 * @param overall  งบรวมทั้งหมดต่อเดือน (0 = ไม่ได้ตั้ง)
 * @param avg      ผลจาก monthlyAvgByType
 *
 * คืนเฉพาะชนิดที่ "ตั้งงบไว้จริง" เพราะชนิดที่ไม่ได้ตั้งงบไม่มีอะไรให้เกิน
 */
export function budgetStatus(types, budgets, overall, avg) {
  const rows = types
    .map((tp) => {
      const budget = n(budgets?.[tp.key]);
      return { ...tp, budget, avg: n(avg?.[tp.key]), over: budget > 0 && n(avg?.[tp.key]) > budget };
    })
    .filter((r) => r.budget > 0);

  const totalBudget = n(overall);
  const total = totalBudget > 0
    ? { budget: totalBudget, avg: n(avg?.total), over: n(avg?.total) > totalBudget }
    : null;

  return { total, rows, overCount: rows.filter((r) => r.over).length + (total?.over ? 1 : 0) };
}
