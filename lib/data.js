...
export const EV_DATA = {
  ...
  // GWM คือบริษัทแม่ของ ORA — รถที่ขายในไทยรุ่นใหม่ใช้ชื่อแบรนด์ว่า GWM
  // ยังคงแบรนด์ ORA ไว้ด้วย เผื่อคนที่บันทึกรถไว้ด้วยชื่อเดิม
  GWM: [
    ['Ora 5 EV Pro', 63.1, 500],
    ['Ora 5 EV Ultra', 63.1, 500],
  ],
  ORA: [
...
export const COST_CATS = {
  ...
  accessory: { label: 'ค่าอุปกรณ์เสริม', color: '#0ea5e9' },
  other: { label: 'ค่าใช้จ่ายอื่นๆ', color: '#64748b' },
};

/**
 * ชนิดค่าใช้จ่ายที่ตั้งงบต่อเดือนแยกกันได้
 *
 * "ค่าชาร์จ" มาจากตารางการชาร์จ ไม่ใช่ตารางต้นทุน จึงไม่มีใน COST_CATS
 * ที่เหลือใช้คีย์ตรงกับ COST_CATS เพื่อให้จับคู่ยอดจริงกับงบได้โดยไม่ต้องแปลงชื่อ
 *
 * ค่าไฟกับค่าใช้จ่ายอื่นๆ ไม่มีในรายการนี้โดยตั้งใจ ตามชนิดที่ผู้ใช้ระบุ
 * ทั้งสองยังบันทึกได้ตามปกติและยังนับรวมในงบรวม แค่ไม่มีงบแยกของตัวเอง
 */
export const BUDGET_TYPES = [
  { key: 'charge', label: 'ค่าชาร์จ', icon: 'bolt', color: 'var(--accent)' },
  { key: 'accessory', label: 'ค่าอุปกรณ์เสริม', icon: 'wallet', color: 'var(--dc)' },
  { key: 'insurance', label: 'ค่าประกันภัย', icon: 'file', color: 'var(--purple)' },
  { key: 'maintenance', label: 'ค่าบำรุงรักษา', icon: 'settings', color: 'var(--ac)' },
  { key: 'tax', label: 'ค่าภาษี', icon: 'coin', color: 'var(--warn)' },
];
...
/**
 * period: true = หน้านี้ใช้ช่วงเวลาจากตัวเลือกบนแถบบนจริง
 * หน้าที่ไม่ได้ตั้งไว้จะไม่แสดงตัวเลือกช่วงเวลาเลย
 * เพราะโชว์ตัวกรองที่กดแล้วไม่มีอะไรเปลี่ยน ทำให้ผู้ใช้สับสนกว่าไม่มี
 * โดยเฉพาะหน้ารายงานที่มีตัวเลือกเดือน/ปีของตัวเองอยู่แล้ว
 */
export const NAV = [
  { href: '/', label: 'แดชบอร์ด', short: 'หน้าหลัก', icon: 'grid', tab: true, period: true, sub: 'ภาพรวมการชาร์จและค่าใช้จ่ายทั้งหมด' },
  { href: '/add', label: 'บันทึกการชาร์จ', short: 'บันทึก', icon: 'plus', sub: 'กรอกข้อมูลการชาร์จแต่ละครั้ง ระบบคำนวณให้อัตโนมัติ' },
  { href: '/history', label: 'ประวัติการชาร์จ', short: 'ประวัติ', icon: 'list', tab: true, period: true, sub: 'ค้นหา กรอง และแก้ไขรายการย้อนหลัง' },
  { href: '/stations', label: 'สถานีชาร์จ', short: 'สถานี', icon: 'map-pin', sub: 'ค้นสถานีชาร์จใกล้ตัวจาก Open Charge Map' },
  { href: '/costs', label: 'ต้นทุนรถ', short: 'ต้นทุน', icon: 'wallet', tab: true, period: true, sub: 'ค่าไฟ ค่าบำรุงรักษา ประกันภัย ภาษี และอื่นๆ' },
