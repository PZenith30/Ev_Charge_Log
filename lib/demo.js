...
export function buildDemoState(settings = {}) {
  ...
  for (let i = 0; i < 26; i++) {
    ...
    // ใส่ส่วนลดบ้างประปราย ให้ข้อมูลตัวอย่างได้ใช้ช่องนี้จริง
    const discount = Math.random() < 0.25 ? 10 + Math.floor(Math.random() * 3) * 10 : 0;
    ...
    sessions.push({
      ...
      fee,
      discount,
      total: Number(Math.max(0, kwh * price + fee - discount).toFixed(2)),
