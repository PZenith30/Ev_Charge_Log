...
import { sDashReading, sDist, sRangeGain, sSoc, sTotal, sEff, sEff100, sKwh100, sBahtKm, sPricePerKwh } from './calc';
...
export function sessionsToCsv(list, carName) {
  const head = [
    ...
    'เลขไมล์ครั้งก่อน (km)', 'เลขไมล์ปัจจุบัน (km)', 'ระยะทางที่ขับมา (km)',
    'SOC ก่อน (%)', 'SOC หลัง (%)', 'SOC เพิ่ม (%)',
    'ระยะทางที่วิ่งได้ ก่อน (km)', 'ระยะทางที่วิ่งได้ หลัง (km)', 'ระยะทางที่วิ่งได้ เพิ่ม (km)',
  ...
  const rows = list.map((s) => {
    ...
    return [
      ...
      s.rangeBefore ?? '', s.rangeAfter ?? '', sRangeGain(s) ?? '',
