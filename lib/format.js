...
/**
 * ชื่อที่ใช้แสดงแทนผู้ใช้บนหน้าจอ
 *
 * ลำดับ: ชื่อที่ตั้งไว้เอง → ส่วนหน้า @ ของอีเมล → คำว่า "ผู้ใช้"
 * ที่ไม่ตกไปใช้อีเมลเต็มเพราะแถบบนกว้างจำกัด และอีเมลเต็มไม่ใช่สิ่งที่อยากโชว์ค้างไว้บนจอ
 * ตัวชื่อที่ตั้งเองเก็บอยู่ใน user_metadata ของ Supabase Auth ไม่ใช่ตาราง settings
 */
export function displayNameOf(user) {
  const saved = (user?.user_metadata?.display_name || '').trim();
  if (saved) return saved;
  const local = (user?.email || '').split('@')[0].trim();
  return local || translate('ผู้ใช้');
}

/** ชื่อที่ผู้ใช้ตั้งไว้จริง — ว่างแปลว่ายังไม่เคยตั้ง (ต่างจาก displayNameOf ที่เดาจากอีเมลให้) */
export const savedNameOf = (user) => (user?.user_metadata?.display_name || '').trim();
 ...
 */
export const dateLocale = () => (getLang() === 'en' ? 'en-GB' : 'th-TH');
