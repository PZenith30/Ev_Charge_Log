...
export const sessionFromDb = (r) => ({
  ...
  rangeBefore: num(r.range_before),
  rangeAfter: num(r.range_after),
  kwh: num(r.kwh) ?? 0,
  price: num(r.price),
  fee: num(r.fee),                 // ในหน้าจอเรียกว่า "ค่าปรับ" ชื่อคอลัมน์คงเดิมเพื่อไม่ให้ข้อมูลเก่าอ่านไม่ได้
...
export const sessionToDb = (s, userId) => ({
  ...
  range_before: num(s.rangeBefore),
  range_after: num(s.rangeAfter),
  kwh: num(s.kwh) ?? 0,
  price: num(s.price),
  fee: num(s.fee),
...
export const settingsFromDb = (r) => {
  ...
  return {
    ...
    // คอลัมน์ jsonb — ผู้ใช้ที่ยังไม่ได้รัน migration-005 จะได้ undefined ตรงนี้
    // จึงตกเป็นอ็อบเจ็กต์ว่าง แปลว่า "ยังไม่ได้ตั้งงบแยกชนิด" ซึ่งเป็นค่าที่ถูกต้อง
    budgets: r.budgets && typeof r.budgets === 'object' ? r.budgets : {},
