...
export function rangeText(key, range) {
  ...
  if (range.from === range.to) return a.toLocaleDateString('th-TH', opt);
  const sameYear = a.getFullYear() === b.getFullYear();
  const sameMonth = sameYear && a.getMonth() === b.getMonth();
  if (sameMonth) {
    return `${a.getDate()} – ${b.toLocaleDateString('th-TH', opt)}`;
  }
  return `${a.toLocaleDateString('th-TH', sameYear ? { day: 'numeric', month: 'short' } : opt)} – ${b.toLocaleDateString('th-TH', opt)}`;
}
