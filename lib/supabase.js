...
export function dbErrorText(error) {
  ...
  if (msg.includes('permission denied')) {
    // ขาด GRANT ระดับตาราง ไม่ใช่ปัญหา RLS — แก้ด้วยการรัน supabase/schema.sql ซ้ำ
    return 'ฐานข้อมูลยังไม่ได้ให้สิทธิ์ role authenticated — เปิด Supabase SQL Editor แล้วรัน supabase/schema.sql ซ้ำอีกครั้ง';
  }
  if (msg.includes('row-level security')) {
    return 'ไม่มีสิทธิ์เขียนข้อมูลนี้ — ตรวจว่ารัน schema.sql ครบแล้วหรือยัง';
  }
  // PostgREST แยกกรณี "ไม่มีคอลัมน์" กับ "ไม่มีตาราง" — บอกวิธีแก้ให้ตรงกรณี
  if (msg.includes('column') && (msg.includes('schema cache') || msg.includes('does not exist'))) {
    const col = /'([^']+)' column/.exec(error.message || '');
    return `ฐานข้อมูลยังไม่มีคอลัมน์${col ? ` "${col[1]}"` : ''} — เปิด Supabase SQL Editor แล้วรัน supabase/schema.sql เวอร์ชันล่าสุด (หรือไฟล์ migration ล่าสุดในโฟลเดอร์ supabase/)`;
  }
